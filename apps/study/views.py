...
from .utils import (
    ...
    create_daily_goal_notification,
    create_study_time_notification,
    create_friend_request_notification,
    create_friend_accept_notification,
    create_daily_mission_notification
)
import random
...
def study_home(request):
    ...
    # 소수점 반올림
    daily_study_time = round(daily_study_time, 1)
    
    # 시간, 분, 초로 분리
    study_hours = int(daily_study_time // 60)
    study_minutes = int(daily_study_time % 60)
    study_seconds = int((daily_study_time * 60) % 60)
    
    # 일일 목표 대비 달성률 계산
    daily_achievement_rate = min(100, int((daily_study_time / user_profile.daily_goal) * 100)) if user_profile.daily_goal > 0 else 0
