...
from .models import StudyProgress, StudySession, ReviewSchedule, Notification, UserNotificationSettings, StudyNotification, StudyPlan, Friendship, FriendRequest
...
def create_daily_goal_notification(user):
    """일일 목표 달성 알림 생성"""
    message = f'축하합니다! 오늘의 학습 목표를 달성했습니다! 🎯'
    return StudyNotification.objects.create(
        user=user,
...
def create_level_up_notification(user, new_level):
    """레벨 업 알림 생성"""
    message = f'축하합니다! 레벨 {new_level}로 상승했습니다! 🏆'
    return StudyNotification.objects.create(
        user=user,
...
def create_points_notification(user, points):
    """포인트 획득 알림 생성"""
    message = f'포인트 {points}점을 획득했습니다! 💎'
    return StudyNotification.objects.create(
        user=user,
...
def create_study_time_notification(user, study_time):
    """학습 시간 달성 알림 생성"""
    message = f'목표한 학습 시간 {study_time}분을 달성했습니다! ⏰'
    return StudyNotification.objects.create(
        user=user,
        notification_type='study_time',
        message=message
    )

def create_friend_request_notification(user, requester):
    """친구 요청 수신 알림 생성"""
...
def create_friend_accept_notification(user, accepter):
    """친구 요청 수락 알림 생성"""
    message = f'{accepter.username}님이 친구 요청을 수락했습니다! 👥'
    return StudyNotification.objects.create(
        user=user,
...
def create_daily_mission_notification(user, points):
    """데일리 미션 포인트 알림 생성"""
    message = f'데일리 미션을 완료하여 {points}포인트를 획득했습니다! 🎯'
    return StudyNotification.objects.create(
        user=user,
        notification_type='daily_mission',
        message=message
    ) 